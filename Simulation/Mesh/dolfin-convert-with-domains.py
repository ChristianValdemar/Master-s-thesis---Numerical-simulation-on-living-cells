...
def gmsh2xml(ifilename, ofilename):
    ...
    ifile = open(ifilename, "r")
    ofile = open(ofilename, "w")
    sfile = open("subdomains.xml", "w")                      # Added by Chr. Valdemar
    ...
    while line:
        ...
        if line.find("$Elements") == 0:
            ...
            dim_3_count = 0
            vertices_2_used = []
            group = []                                                  # Added by Chr. Valdemar
            local_entity = []                                           # Added by Chr. Valdemar
            vertices_3_used = []
            while line.find("$EndElements") == -1:
                ...
                if elem_type == 2:
                    ...
                    group.extend([element[2+num_tags]])                  # Added by Chr. Valdemar
                    local_entity.extend([element[1+num_tags]])           # Added by Chr. Valdemar
                    dim_2_count += 1
                elif elem_type == 4:
                    if dim < 3:
                        cell_type = "tetrahedron"
    ...
    if dim == 3:
        ...
        vertex_set = sets.Set(vertices_3_used)
        vertices_3_used = None
    elif dim == 2:
        num_cells_counted = dim_2_count
        vertex_set = sets.Set(vertices_2_used)
        vertices_2_used = None
    ...
    # Added by Chr. Valdemar
    write_header_meshfunction(sfile, dim, num_cells_counted)
    for i in range(num_cells_counted):
        write_cell_value(sfile, i, int(local_entity[i]), int(group[i]))
    write_footer_meshfunction(sfile)
...
# Write mesh function header
def write_header_meshfunction(ofile, dim, size):
    ofile.write("""\
<?xml version=\"1.0\"?>
...
def write_cell_value(ofile, vertex, le, v):
    "Write vertex"
